...
class SyncEventHubConsumer(EventHubConsumerClient):
    ...
    def execute_on_event(callback_method):
        ...
        def on_event_receive(partition_context, events):
            ...
            callback_method(partition_context, deserialized_events)
            partition_context.update_checkpoint(events[-1])

        return on_event_receive

    def receive_event(self, callback_method):
        """
        Synchronously receives events from Event Hub and processes them via the provided callback method.
        """
    ...
    async def receive_event_batch(self, callback_method):
        """
        Synchronously receives a batch of events from Event Hub and processes them via the provided callback method.
        """
    ...
    def start_consumer(self, callback_method):
        """
            Starts the event hub consumer by running the receive_event method.
        """
        self.receive_event(callback_method)
